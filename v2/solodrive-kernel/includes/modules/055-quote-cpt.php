 ...
 *
 * Purpose:
 * - Register sd_quote CPT.
 * - Enforce tenant scoping: every sd_quote MUST have meta sd_tenant_id.
 * - Provide a single canonical creator for programmatic usage.
 *
 * Canon:
 * - Quotes are tenant-scoped records.
 * - sd_tenant_id is REQUIRED on all quotes.
 */
final class SD_CPT_Quote {

  public const CPT = 'sd_quote';

  private const NONCE_ACTION     = 'sd_quote_tenant_scope';
  private const NOTICE_TRANSIENT = 'sd_notice_quote_tenant_required';
  ...
  public static function create(int $tenant_id, array $meta = []) {
    ...
    if ($tenant_id <= 0) {
      return new WP_Error('sd_tenant_missing', 'Tenant id is required to create a quote.');
    }
    ...
    if (is_wp_error($quote_id) || (int) $quote_id <= 0) {
      return is_wp_error($quote_id) ? $quote_id : new WP_Error('sd_quote_create_failed', 'Quote creation failed.');
    }

    update_post_meta((int) $quote_id, SD_Meta::TENANT_ID, $tenant_id);
  ...
  public static function render_tenant_metabox(\WP_Post $post) : void {
    ...
    wp_nonce_field(self::NONCE_ACTION, '_sd_quote_tenant_nonce');

    echo '<p class="description">All quotes must belong to a tenant.</p>';
  ...
  public static function save_tenant_metabox(int $post_id, \WP_Post $post) : void {
    ...
    $nonce = isset($_POST['_sd_quote_tenant_nonce']) ? (string) $_POST['_sd_quote_tenant_nonce'] : '';
  ...
  public static function maybe_admin_notice() : void {
    ...
    echo '<div class="notice notice-error"><p><strong>Quote not saved:</strong> sd_tenant_id is required for all quotes. The record was forced to Draft.</p></div>';
  }
}

SD_CPT_Quote::register();